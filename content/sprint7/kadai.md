...
これらの課題を解決するため、**Infrastructure as Code（IaC）** の考え方を導入し、**Terraform（CloudFormationでも可）**を使用してインフラをコードで管理することになりました。

あなたはインフラエンジニアとして、過去のスプリントで構築した環境をTerraformでコード化する必要があります。

## 実施手順

1. IaCで構築するスプリント課題を選択しましょう。
2. 選択したスプリントの要件を満たすコードを作成しましょう。
3. AWS環境を構築しましょう。
...
### 要件1：対象スプリントの選択
以下のスプリント課題から1つを選択し、IaCでインフラを構築する
...
### 要件2：コードの作成
- 選択したスプリントの要件を満たすTerraform（またはCloudFormation）コードを作成する
...
## 構築チェックリスト
- 実装したTerraform（またはCloudFormation）のコードを確認してください。

## 動作確認
- Terraform（またはCloudFormation）を実行し、リソースが作成されること
...
### 問題3：複数環境での利用
複数の環境（開発環境、ステージング環境、本番環境など）で同じ構成のリソースを用意する際、環境ごとにコードをコピーして管理すると、変更時の修正漏れや管理の煩雑さが問題になります。Terraform（またはCloudFormation）ではこの問題をどのように解決できるかを考え、説明してください。
