...
### 要件2：スケジュール実行
- 毎月1日の午前9時に自動的にワークフローが開始されること

### 要件3：ワークフローの構築
- 複数の処理を順番に実行するワークフローが構築されていること

### 要件4：売上データ集計関数
- 売上データの集計関数が作成されていること
...
### 要件1：売上データの準備
- ストレージに売上データ（CSVファイル）が配置されていること

### 要件2：スケジュール実行
- 毎月1日の午前9時に自動的にワークフローが開始されること
- スケジュールに基づいてイベントが発火されること

### 要件3：ワークフローの構築
- 複数の処理を順番に実行するワークフローが構築されていること
- 各処理の成功・失敗に応じて適切にハンドリングされること
- 処理の状態が可視化できること

### 要件4：売上データ集計関数
- ストレージからCSVファイルを読み込めること
- 商品カテゴリ別の売上合計が集計されること
- 日別の売上合計が集計されること
- 総売上金額が集計されること
- 集計結果が次の処理に渡されること

### 要件5：レポート作成関数
- 集計データを受け取り、レポートが作成されること
- 作成したレポートがストレージに保存されること
- レポート作成完了後、メッセージがキューに送信されること

### 要件6：メッセージキューによる非同期処理
- レポート作成関数からメール送信関数への連携がメッセージキューで行われること
- メッセージキューにメッセージが入ると、自動的にメール送信関数が起動すること

### 要件7：メール送信関数
- キューからメッセージを受け取り、レポートがメールで送信されること
- 宛先が設定で変更可能であること
- メール本文にレポートの概要が記載されていること

### 要件8：エラーハンドリング
- メール送信に失敗した場合、一定回数リトライされること
- リトライしても失敗したメッセージが、失敗用キューに移動されること
- 失敗したメッセージが後から確認・再処理できること

## 動作確認
- ワークフローを手動で実行し、各処理が順番に実行されること
- レポートがストレージに保存されること
- メール送信関数が起動し、メールが送信されること
- スケジュール設定した時刻にワークフローが自動開始されること
- メール送信失敗時に、指定回数リトライされ、失敗したメッセージが失敗用キューに移動すること

## 理解度の確認
この課題の理解度を確認するため、以下の内容を言語化してみましょう。

### 問題1：Step Functionsの特徴
AWS Step Functionsとは何か、その特徴と、複数のLambda関数を直接連携させる方法と比較した場合のメリットについて説明してください。

### 問題2：EventBridgeの役割
Amazon EventBridgeとは何か、その役割と、今回の構成においてどのように活用されているか説明してください。また、EventBridgeを使用することで実現できるユースケースを他にも挙げてください。

### 問題3：SQSとデッドレターキュー
Amazon SQSとは何か、その特徴を説明してください。また、デッドレターキュー（DLQ）の役割と、なぜ今回の構成で必要とされるのかについても説明してください。

### 問題4：同期処理と非同期処理
今回の構成では、Step Functionsによる同期的なワークフロー実行と、SQSを使った非同期処理が組み合わされています。同期処理と非同期処理それぞれの特徴と、どのような場面で使い分けるべきか説明してください。
