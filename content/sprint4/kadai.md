...
### 要件1：静的ファイルの配置
- 静的サイトのファイル（HTML/CSS）がオブジェクトストレージに格納されていること
- オブジェクトストレージへの直接アクセスが拒否されていること
- CDN経由でのみコンテンツにアクセスできること

### 要件2：CDNの利用
- CDNが設定されていること
- オリジンとしてオブジェクトストレージが指定されていること

### 要件3：独自ドメインの設定
- 独自ドメインが取得されていること
- DNSが設定されていること
- 独自ドメインでCDNにアクセスできること

### 要件4：HTTPS通信の実現
- サーバ証明書が取得されていること
- CDNに対してサーバ証明書が設定されていること

## 動作確認
- 独自ドメインでアクセスしたとき、コーポレートサイトが表示されること
    - 例：`https://<独自ドメイン>/`
...
### 問題1：通信経路の確認
今回の構成において、独自ドメインを使ってリクエストを発行してから、レスポンスが帰るまでの通信の流れを説明してください。

### 問題2：CloudFrontのメリット・デメリット
EC2インスタンスでサーバを構築する方法と比較して、CDN + オブジェクトストレージを利用した今回の構成はどのようなメリット・デメリットがあるか、具体的な利用シーンを交えて説明してください。

### 問題3：HTTPS通信のメリット
通信をHTTPS化することでどのようなメリットがあるか、具体的な利用シーンを意識しながら、具体的な仕組みをあわせて説明してください。
