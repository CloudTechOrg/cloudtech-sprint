...
| 担当 | チーム | 役割・タスク |
| :--- | :--- | :--- |
| **開発担当** | 開発チーム | ・アプリケーション開発<br>・Dockerfileの作成 |
| **あなた** | インフラチーム | ・AWS構成図作成<br>・AWS環境構築<br>・CI/CD用の設定ファイル作成 |
...
### 1. 初回デプロイの確認
パイプラインを手動で実行し、アプリケーションがデプロイされることを確認します。

```bash
# おみくじを引く
curl http://<エンドポイント>/omikuji
```
...
### 2. 自動デプロイの確認
1. Forkしたリポジトリのコードを変更する（例：おみくじの結果メッセージを変更）
2. 変更をGitHubにプッシュする
3. パイプラインが自動的に実行されることを確認する
4. デプロイ完了後、変更が反映されていることを確認する
