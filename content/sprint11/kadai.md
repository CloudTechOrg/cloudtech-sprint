...
> **Note**
> QuickSightは無料利用枠の対象外であり、利用に応じて料金が発生します。そのため、本要件の実施は任意とします。Athenaでのクエリ実行（要件4）までの確認ができていれば、データ分析基盤の基本的な構築は完了です。
...
※ QuickSightは料金が発生するため、本要件の実施は任意です。
...
### 問題1：データ分析基盤の全体像
今回構築したデータ分析基盤について、RDSからQuickSightでダッシュボードを表示するまでのデータの流れを、各AWSサービスの役割を明確にしながら説明してください。

### 問題2：ETL処理とAWS Glue
ETL（Extract, Transform, Load）処理とは何か説明してください。また、AWS Glueを使用してETL処理を行うメリットについて、従来のETLツールと比較しながら説明してください。

### 問題3：データレイクとデータウェアハウス
データレイクとデータウェアハウスの違いについて説明してください。また、S3をデータレイクとして使用するメリットについても説明してください。

### 問題4：Athenaの特徴
Amazon Athenaとは何か、その特徴を説明してください。また、RDSなどのデータベースに直接SQLを実行する方法と比較した場合のメリット・デメリットについても説明してください。
