...
※ ＜ロードバランサーのドメイン名＞は実際の値に置き換えてください
```
curl -X POST http://＜ロードバランサーのドメイン名＞/api/persons \
...
```
curl http://＜ロードバランサーのドメイン名＞/api/persons
```
...
### 問題1：通信経路の確認
今回の構成において、リクエストを発行してから、レスポンスが帰るまでの通信の流れを説明してください。

### 問題2：コンテナのメリット
EC2インスタンスに直接ミドルウェアやアプリケーションをインストールして実行する方法と比較して、ECS/Fargateを用いて、コンテナでアプリケーションを実行するメリットについて、具体的な利用シーンを交えながら説明してください。

### 問題3：ECSの構成要素
ECSを構成する以下の要素について、それぞれの役割を説明したうえで、ECSでコンテナを起動する際にこれらがどのように連携するかを説明してください。
- クラスター
- タスク定義
- タスク
- サービス
